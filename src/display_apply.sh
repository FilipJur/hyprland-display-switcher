...
# It contains both monitor= (layout) and monitorv2 {} (color management) blocks.
...
HEADER
    ...
    # Common monitorv2 blocks for color management (applied to all modes)
    # These params are NOT supported by the legacy monitor= inline syntax.
    cat >> "$GENERATED_CONFIG" <<EOF
# Color management for main monitor (DP-2) — 10-bit wide-gamut SDR
monitorv2 {
    output = DP-2
    bitdepth = 10
    cm = dcip3
...
# Color management for TV (DP-1) — 10-bit HDR with true OLED blacks
monitorv2 {
    output = DP-1
    bitdepth = 10
    cm = hdr
...
EOF
    ...
    # Mode-specific layout (legacy monitor= syntax)
    case "$mode" in
        monitor)
            cat >> "$GENERATED_CONFIG" <<EOF
# Monitor mode: DP-2 active, DP-1 disabled
monitor = DP-2,${MONITOR_RES},0x0,1,bitdepth,10,cm,dcip3
monitor = DP-1,disable
EOF
            ;;
        extend)
            cat >> "$GENERATED_CONFIG" <<EOF
# Extend mode: both active, TV to the left of main monitor
monitor = DP-2,${MONITOR_RES},0x0,1,bitdepth,10,cm,dcip3
monitor = DP-1,${TV_RES},-2560x0,${TV_SCALE},bitdepth,10,cm,hdr
EOF
            ;;
        mirror)
            cat >> "$GENERATED_CONFIG" <<EOF
# Mirror mode: DP-2 active, DP-1 mirroring DP-2
monitor = DP-2,${MONITOR_RES},0x0,1,bitdepth,10,cm,dcip3
...
EOF
        ...
        tv)
            cat >> "$GENERATED_CONFIG" <<EOF
# TV mode: DP-2 disabled, DP-1 active
monitor = DP-2,disable
monitor = DP-1,${TV_RES},0x0,${TV_SCALE},bitdepth,10,cm,hdr
EOF
            ;;
        monitor_only)
            # Internal helper: enable only DP-2 (used for retry reset)
            cat >> "$GENERATED_CONFIG" <<EOF
# Reset: DP-2 only
monitor = DP-2,${MONITOR_RES},0x0,1,bitdepth,10,cm,dcip3
monitor = DP-1,disable
EOF
            ;;
        tv_only)
            # Internal helper: enable only DP-1 (used for retry reset)
            cat >> "$GENERATED_CONFIG" <<EOF
# Reset: DP-1 only
monitor = DP-2,disable
monitor = DP-1,${TV_RES},0x0,${TV_SCALE},bitdepth,10,cm,hdr
